...
public class SseApiController {
    ...
    public ResponseBodyEmitter connect(
        ...
        var emitter = new SseEmitter(1000L * 60); // ms 단위로 리 커넥션
    ...
    public void pushEvent(
        ...
        var emmiter = userConnection.get(userSession.getUserId().toString());

        var event = SseEmitter
                .event()
                .data("hello"); // event에 별도 이름을 정해주지 않으면, 클라이언트에서는 onmessage의 이벤트 이름으로 수신
